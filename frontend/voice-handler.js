...
class VoiceHandler {
    ...
    onRecordingStart() {
        ...
        recordingText.textContent = this.getTranslation('listening', this.currentLanguage);
    ...
    onRecordingEnd() {
        ...
        recordingText.textContent = this.getTranslation('tap-to-speak', this.currentLanguage);
    ...
    async processHealthQuery(text) {
        try {
            ...
            window.app.hideLoading();
            window.app.showError('Failed to process your query. Please try again or use text input.');
        }
    }
    ...
    speakResponse(text, language = null) {
        ...
        utterance.lang = this.languageMappings[language || this.currentLanguage] || 'en-IN';
        utterance.rate = 0.9;
        utterance.pitch = 1;
