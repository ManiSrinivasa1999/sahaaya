    ...
    event.waitUntil(
        caches.keys().then(cacheNames => {
            return Promise.all(
                cacheNames
                    .filter(cacheName => {
                        return cacheName.startsWith('sahaaya-') && 
...
function isAPIRequest(url) {
    return url.pathname.startsWith('/api/') || 
           ...
           url.pathname.match(/^\/(smart-process|offline-guidance|emergency-protocol|connectivity-status)$/);
}

// Check if request is for static asset
function isStaticAsset(url) {
    return url.pathname.match(/\.(css|js|png|jpg|jpeg|svg|ico|woff|woff2|ttf|json)$/) ||
...
async function handleNavigationRequest(request) {
    try {
        ...
        console.log('Sahaaya SW: Network failed for navigation, serving cached index.html');
...
function getOfflineFallback(pathname) {
    const fallbackResponses = {
        ...
        '/offline-guidance': {
            guidance: 'Offline mode active. Basic health guidance available. For serious symptoms, seek immediate medical attention.',
        ...
        '/emergency-protocol': {
            emergency_guidance: {
                guidance: 'Call emergency services immediately: 108 for medical emergencies, 100 for police.',
                emergency_contact: '108',
        ...
        '/smart-process': {
            guidance: 'Currently offline. For medical emergencies, call 108 immediately. For non-urgent health concerns, please connect to the internet for detailed guidance.',
            processing_mode: 'offline',
...
async function storeFailedQuery(query) {
    const failedQueries = await getFailedQueries();
    failedQueries.push({ ...query, id: Date.now(), timestamp: new Date().toISOString() });
...
self.addEventListener('push', event => {
    if (event.data) {
        ...
        event.waitUntil(
            self.registration.showNotification(data.title || 'Sahaaya Health', options)
        );
    }
});
